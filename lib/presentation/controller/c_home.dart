...
class CHome extends GetxController {
  ...
  getAnalysis(String idUser) async {
    ...
    _todayPercent.value = isSame
        ...
        : isPlus
        ? '+${percent.toStringAsFixed(1)}% di banding dengan kemarin'
        : '${percent.toStringAsFixed(1)}% di banding dengan kemarin';
    ...
    _differentMonth.value = (monthIncome - monthOutcome).obs();
    bool isSamMonth = monthIncome.isEqual(monthOutcome);
    bool isPlusMonth = monthIncome.isGreaterThan(monthOutcome);
    double divider = (monthIncome + monthOutcome) == 0 ? 1 : (monthIncome + monthOutcome);
    double percentMonth = (differentMonth / divider) * 100;
    _percentIncome.value = percentMonth.toStringAsFixed(1);
    _monthPercent.value = isSamMonth
        ? 'Pemasukan\n100% sama\ndengan Pengeluaran'
        : isPlusMonth
